...
public class JedisIndex {
	...
	public Map<String, Integer> getCounts(String term) {
		...
		for (String url : urls) {
			...
			counts.put(url, count != null ? Integer.parseInt(count) : 0);
		}
	...
	public void indexPage(String url, Elements paragraphs) {
		// Create a TermCounter for this URL
		TermCounter tc = new TermCounter(url);

		// Process each paragraph and record the counts of terms
		tc.processElements(paragraphs);

		// Store the TermCounter in Redis
		String tcKey = termCounterKey(url);
		Transaction t = jedis.multi();

		for (String term : tc.keySet()) {
			t.hset(tcKey, term, tc.get(term).toString());
